...
public class KeywordHelpers {
    ...
    public static ArrayList<Twitpost> getCelebMentionsOfKeyword(Twit twit, Keyword keyword){
        ...
        List twitposts = HibernateUtil.getSession().createSQLQuery("SELECT * FROM twitpost WHERE MATCH(post) AGAINST('\""+keyword.getKeyword()+"\"' IN BOOLEAN MODE) AND twitid='"+twit.getTwitid()+"' ORDER BY created_at DESC").addEntity(Twitpost.class).list();
